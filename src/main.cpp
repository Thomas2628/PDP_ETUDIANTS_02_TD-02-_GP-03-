...
const char* wifi_ssid = "iPhone de Thomas";
const char* wifi_password = "thomaspel8";
...
void setup() {
  ...
  // Connecting to MQTT and Send data to the broker with MQTT
  if (mqtt_client.connect(mqtt_user, mqtt_pass, client_id)) {
    mqtt_client.publish("TD02_GP03/temp", String(temp_measure).c_str());
    mqtt_client.publish("TD02_GP03/relhum", String(relative_humidity_measure).c_str());
  }
